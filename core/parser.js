...
class Parser {
  ...
  isNextTokenPunctuation(punc) {
    ...
    return token && token.type === 'PUNCTUATION' && token.value === punc;
  }
  ...
  isNextTokenKeyword(kw) {
    const token = this.lexer().peek();
    return token && token.type === 'KEYWORD' && token.value === kw;
  }
  ...
  parseNodeLiteral() {
    ...
    if (nodeLiterals['EXP_PUNC'][token.value]) {
      const nodeliteral = nodeLiterals['EXP_PUNC'][token.value];
      return nodeliteral.getNode.call(this);
    }
  ...
  parseVarname() {
    return this.lexer().peek().type === 'VARIABLE'
  ...
  parseAst() {
    ...
    if (token.type === 'VARIABLE') {
      const callsutraNodeLiteral = nodeLiterals['CALL_SUTRA'];
