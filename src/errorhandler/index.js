const message = {
  ...
  chakraIncrementAndDecrementMsg: () => 'Invalid decrement or increment operation',
  invalidFileMsg: () => 'File with invalid extension',
  invalidAssignment: () => 'Cannot assign value to  sutra call',
  invalidSuchiIndexTypeMsg: (arg) => `Typeof index given for suchi "${arg}" must be a number`,
  suchiIndexDoesNotExistMsg: (arg) => `Index given for suchi "${arg}" does not exist`,
  varDoesNotExist: (type, name) => `${type} "${name}" is undefined`,
  sutraAlreadyExist: (name, scope) => `sutra with name "${name}" already exists within the "${scope}" scope`,
