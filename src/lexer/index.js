...
class Lexer {
  ...
  readIdentifier () {
    ...
    return {
      type: this.isKeyword(identifier) ? symboltable.KEYWORD : symboltable.VARIABLE,
      value: identifier
    };
  }
  ...
  devtoengNum(str){
    ...
    .split("")
    .map((char) => {
        const englishDigit = this.digitMapping.findIndex((digit) => digit === char)
  ...
  readNext () {
    ...
    if (ch === symboltable.SYM.DBL_QUOTE) return this.readString(symboltable.SYM.DBL_QUOTE);
    if (ch === symboltable.SYM.SGL_QUOTE) return this.readString(symboltable.SYM.SGL_QUOTE);
    if (this.isDigit(ch)) return this.readNumber();
    if (this.isIdentifier(ch)) return this.readIdentifier();
    if (this.isPunctuation(ch)) return { type: symboltable.PUNCTUATION, value: this.inputStream.next() };
    if (this.ivadrator(ch)) return { type: symboltable.OPERATOR, value: this.readWhile(this.ivadrator) };
