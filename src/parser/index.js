...
class Parser {
  ...
  isNextTokenPunctuation (punc) {
    ...
    return token && token.type === symboltable.PUNCTUATION && (token.value === punc);
  }
  ...
  skipPunctuation (punc) {
    ...
    else this.throwError(errorhandler.unexpectedtoken(this.getCurrentTokenValue()));
  }
  ...
  parseAnd () {
    return this.parseWhile([symboltable.SYM.AND], this.parseGreaterLesserEquality);
  }

  parseGreaterLesserEquality () {
    const operatorList = [
      symboltable.SYM.L_THAN, symboltable.SYM.G_THAN, symboltable.SYM.G_THAN_OR_EQ,
      symboltable.SYM.L_THAN_OR_EQ, symboltable.SYM.EQ, symboltable.SYM.NOT_EQ
    ];
  ...
  parsePlusMinus () {
    return this.parseWhile([symboltable.SYM.PLUS, symboltable.SYM.MINUS], this.parseMultiplyDivisionRemainder);
  }

  parseMultiplyDivisionRemainder () {
    return this.parseWhile([symboltable.SYM.MULTIPLY, symboltable.SYM.DIVIDE, symboltable.SYM.REMAINDER], this.parseNodeLiteral);
  }
  ...
  isNotEndOfBlock () {
    return this.isNotEndOfFile() && (this.lexer().peek().value !== symboltable.SYM.R_PAREN);
  }

  parseVarname () {
    return (this.lexer().peek().type === symboltable.VARIABLE)
      ? this.lexer().next().value
      : this.lexer().throwError(errorhandler.unexpectedtoken(this.lexer().peek().value));
  }
